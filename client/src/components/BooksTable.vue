<template>
	<div class="py-6">
		...
		<!-- Add book button -->
		<div>
			<button type="button" class="bg-green-500 hover:bg-green-700 text-white px-4 py-2 mb-4 rounded" v-on:click="openModal('add')">
				Add VPS
			</button>
		</div>
		...
		<!-- Modal window -->
		<Modal v-if="showModal" v-on:closeModal="onCloseModal" v-bind="modalState"> </Modal>
	</div>
</template>
...
export default {
	...
	methods: {
		...
		openModal(m, index = -1) {
			...
			switch (m) {
				...
				case 'delete':
					...
					break
				default:
					throw new TypeError(`State was ${m} which is not add, edit or delete`)
			}
			this.showModal = true
		},
		/**
		...
		onCloseModal(modalPayload) {
			...
			if (Object.keys(modalPayload).length === 0 && modalPayload.constructor === Object) {
				return
			}
